...
def filter_rows(event):
    ...
    responsibilities
    """
    if event.sheet_name == "List 1" and event.row["Date of Contact"] == "":
        yield event.from_event(event, filter=1)

    elif event.sheet_name == "List 3" and event.row["Date of referral"] == "":
        yield event.from_event(event, filter=1)

    elif event.sheet_name == "List 5" and event.row["Strategy discussion initiating Section 47 "
                                                    "Enquiry Start Date"] == "":
        yield event.from_event(event, filter=1)

    elif event.sheet_name == "List 9" and event.row["Date of Birth"] == "":
        yield event.from_event(event, filter=1)

    elif event.sheet_name == "List 10" and event.row["Date of Decision that Child Should be "
                                                     "Placed for Adoption"] == "":
        yield event.from_event(event, filter=1)

    elif event.sheet_name == "List 11" and event.row["Date enquiry received"] == "":
        yield event.from_event(event, filter=1)
...
def save_tables(stream, output):
    ...
    for event in stream:
        if isinstance(event, events.StartContainer):
            ...
            dataset = event.data
            #dataset.title = event.sheet_name
