    ...
    try:
        ...
        for table_name, expected_columns in column_names.items():
            ...
            if len(failed_matches) == len(column_names.items()):
                return event.from_event(event, match_error=f"Failed to find a set of matching columns headers for "
...
def match_config_to_cell(event, config):
    ...
    except (AttributeError, KeyError):  # Raise in case there is no config item for the given table name and cell header
        return event
    except Exception as ex:
        log.exception(f"Error occurred in {event.filename}")
