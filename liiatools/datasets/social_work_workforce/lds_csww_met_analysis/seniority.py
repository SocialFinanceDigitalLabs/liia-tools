...
def create_seniority_table(data: pd.DataFrame) -> pd.DataFrame:
    """
...
def _find_min_max_year_population_growth_table(population_growth_table: pd.DataFrame) -> tuple[int, int]:
    """
...
def seniority_forecast_04(data: pd.DataFrame, population_growth_table: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    """
    min_year, max_year = _find_min_max_year_population_growth_table(population_growth_table)
    ...
    for borough in boroughs:
        for year in range(min_year, max_year):
            data.loc[data["LA"] == borough, str(year+1)] = (
                    data[str(year)] /
                    float(population_growth_table.loc[population_growth_table["LEAName"] == borough, str(year)]) *
                    float(population_growth_table.loc[population_growth_table["LEAName"] == borough, str(year+1)])
            )
...
def seniority_comp(data: pd.DataFrame) -> pd.DataFrame:
    """
...
def seniority_forecast_5c(data: pd.DataFrame, data_comp: pd.DataFrame) -> pd.DataFrame:
    """
    Merge the OrgRole and Seniority information from data_comp into data

    :param data: Dataframe containing input data
    :param data_comp: Dataframe containing the data to be merged with "data" dataframe
    :return: Dataframe with updated OrgRole, OrgRoleName, SeniorityCode and SeniorityName columns
    """
    data_comp = data_comp.sort_values(by=["SWENo", "YEAR"]).reset_index()
    data = data.sort_values(by=["SWENo", "YEAR"]).reset_index()

    data["OrgRole"] = data_comp["OrgRole"]
    data["OrgRoleName"] = data_comp["OrgRoleName"]
    data["SeniorityCode"] = data_comp["SeniorityCode"]
    data["SeniorityName"] = data_comp["SeniorityName"]
