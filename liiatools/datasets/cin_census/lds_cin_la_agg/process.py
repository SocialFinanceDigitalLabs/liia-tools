...
def read_file(input, dates):
    ...
    """
    flatfile = pd.read_csv(input, parse_dates=dates)
    return flatfile
...
def merge_la_files(flat_output, dates, flatfile):
    ...
    if old_file.is_file():
        old_df = pd.read_csv(old_file, parse_dates=dates)
...
def filter_flatfile(flatfile, filter):
    ...
    """
    factors = flatfile[flatfile["Type"] == filter]
...
def split_factors(factors):
    ...
    """
    factor_cols = (
        factors.Factors.str.split(",", expand=True).stack().str.get_dummies().sum(level=0)
    )
