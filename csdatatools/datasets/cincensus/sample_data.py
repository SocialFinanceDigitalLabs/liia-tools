...
def generate_sample_header(**opts):
    ...
    yield from TE(tag='Release', text=f"{random.randint(100, 999):03}")
    yield from TE(tag='SerialNo', text=f"{random.randint(100, 999):03}")
...
def generate_child(configuration=None, **opts):
    ...
    for d in cin_details:
        ...
        yield from TE(tag='ReferralNFA', text='true')
