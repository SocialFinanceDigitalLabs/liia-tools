...
def test_deduplicate():
    test_df_1 = pd.DataFrame({"Column 1": [1, 2], "Column 2": ["a", "a"], "Column 3": ["Answer 1", "Answer 2"]})
    sort_order = ["Column 1"]
    dedup_1 = ["Column 2"]
...
def test_filter_flatfile():
    test_df_1 = pd.DataFrame({"Type": ["Type 1", "Type 2"], "Type 1 data": ["a", None], "Type 2 data": [None, "b"]})
...
def test_time_between_date_series():
    test_df_1 = pd.DataFrame({"date_series_1": ["2022-01-01", "2022-01-01"], "date_series_2": ["2021-01-01", "2020-01-01"]}, dtype='datetime64[ns]')
    output_series_1 = agg_process._time_between_date_series(test_df_1["date_series_1"], test_df_1["date_series_2"], years=1)
    assert list(output_series_1) == [1, 2]
    output_series_2 = agg_process._time_between_date_series(test_df_1["date_series_1"], test_df_1["date_series_2"], days=1)
