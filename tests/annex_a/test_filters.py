...
def test_match_column_name():
    ...
    assert _match_column_name('Child ID', 'Child Unique ID', [r'/.*child.*id.*/i']) == 'Child Unique ID'
    assert _match_column_name('child id', 'Child Unique ID', [r'/.*child.*id.*/i']) == 'Child Unique ID'
...
def test_add_sheet_name_exact():
    ...
    stream = add_sheet_name([
        events.StartTable(column_headers=list_1_columns)
...
def test_add_sheet_name_extra_header():
    cfg = Config()
    stream = add_sheet_name([
        events.StartTable(column_headers=list_1_columns + ['Extra Header 1', 'Extra Header 2'])
    ], config=cfg['datasources'])
    stream = list(stream)
    assert stream[0].sheet_name == 'List 1'
    assert stream[0].matched_column_headers == set(list_1_columns)
...
def test_add_sheet_name_regex():
    ...
    my_cols[0] = "Child ID"
    stream = add_sheet_name([
        events.StartTable(column_headers=my_cols)
    ], config=cfg['datasources'])
    stream = list(stream)
