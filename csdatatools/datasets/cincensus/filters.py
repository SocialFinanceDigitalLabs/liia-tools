...
@streamfilter(
    check=type_check(events.TextNode), fail_function=pass_event
)
def clean_xml_date(event):
    if event.get('config') and "date" in event.config:
...
def clean_categories(event):
    if event.get('config') and "category" in event.config:
