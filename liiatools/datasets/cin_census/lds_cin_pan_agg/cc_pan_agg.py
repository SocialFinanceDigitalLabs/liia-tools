...
from liiatools.datasets.cin_census.lds_cin_pan_agg import cc_pan_agg_config
...
def merge_agg_files(flat_output, la_name, new_df):
...
def export_flatfile(flat_output, df):
...
def export_factfile(analysis_output, df):
    ...
    output_path = Path(analysis_output, f"pan_London_factors.csv")
    df.to_csv(output_path, index=False)
...
def merge_ref_s17(ref, s17):
    ...
    ref_s17 = data_s17[
        (
            (data_s17["days_to_s17"] <= cc_pan_agg_config.ref_assessment)
...
def merge_ref_s47(ref, s47):
    ...
    ref_s47 = data_s47[
        (
            (data_s47["days_to_s47"] <= cc_pan_agg_config.ref_assessment)
...
def journey_inputs(df):
    ...
    s47_cpp = s47_cpp[
        (
            ...
            & (s47_cpp["icpc_to_cpp"] <= cc_pan_agg_config.icpc_cpp_days)
        )
        | (
            (s47_cpp["s47_to_cpp"] >= 0)
            & (s47_cpp["s47_to_cpp"] <= cc_pan_agg_config.s47_cpp_days)
        )
    ]
