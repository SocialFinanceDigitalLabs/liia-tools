...
from liiatools.datasets.sen2.lds_sen2_clean.parse import dom_parse
from liiatools.datasets.sen2.lds_sen2_clean.schema import Schema

from liiatools.datasets.sen2.lds_sen2_clean import (
    file_creator,
...
from liiatools.datasets.sen2.lds_sen2_la_agg import configuration as agg_config
from liiatools.datasets.sen2.lds_sen2_la_agg import process as agg_process

# Dependencies for pan_agg()
from liiatools.datasets.sen2.lds_sen2_pan_agg import configuration as pan_config
from liiatools.datasets.sen2.lds_sen2_pan_agg import process as pan_process
...
@click.group()
def sen2():
    """Functions for cleaning, minimising and aggregating sen2 files"""
    pass
...
def cleanfile(input, la_code, la_log_dir, output):
    """
    Cleans input sen2 xml files according to config and outputs cleaned csv files.
    ...
    if (
        ...
        save_incorrect_year_error(input, la_log_dir, retention_period=years_to_go_back)
        return
    ...
    # Output result
    stream = sen2_record.message_collector(stream)
    data = sen2_record.export_table(stream)
...
def la_agg(input, flat_output, analysis_output):
    """
    Joins data from newly cleaned sen2 file (output of cleanfile()) to existing sen2 data for the depositing local authority
    :param input: should specify the input file location, including file name and suffix, and be usable by a Path function
...
def pan_agg(input, la_code, flat_output, analysis_output):
    """
    Joins data from newly merged sen2 file (output of la-agg()) to existing pan-London sen2 data and creates analytical outputs
    :param input: should specify the input file location, including file name and suffix, and be usable by a Path function
    :param la_code: should be a three-letter string for the local authority depositing the file
    :param flat_output: should specify the path to the folder for the main flatfile output
    :param analysis_output: should specify the path to the folder for the additional analytical outputs
