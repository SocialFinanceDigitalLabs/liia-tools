...
def add_sheet_name(event, schema: DataSchema):
    """
    ...
    if not headers:
        ...
        sheet_name = schema.get_table_from_headers(event.headers)

    if sheet_name:
        return event.from_event(
            event, sheet_name=sheet_name, table_spec=schema.column_map[sheet_name]
        )
    else:
        return event
...
def convert_column_header_to_match(event, schema: DataSchema):
    ...
    :return: An updated list of event objects
    """
    if hasattr(event, "sheet_name") and hasattr(event, "header"):
        column_config = schema.table.get(event.sheet_name)
...
def match_config_to_cell(event, schema: DataSchema):
    ...
    """
    if hasattr(event, "sheet_name") and hasattr(event, "header"):
        table_config = schema.table.get(event.sheet_name)
...
def collect_tables(stream):
    ...
    for event in stream:
        if isinstance(event, events.StartRow) and hasattr(event, "sheet_name"):
            table_data = dataset.setdefault(event.sheet_name, [])
