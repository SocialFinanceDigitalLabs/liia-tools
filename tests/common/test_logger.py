...
def test_blank_error_check():
    stream = logger.blank_error_check(
        [
            events.Cell(config_dict={"canbeblank": False}, cell="", formatting_error="0"),
            events.Cell(config_dict={"canbeblank": False}, cell=None, formatting_error="0"),
            events.Cell(config_dict={"canbeblank": False}, cell="", formatting_error="1"),
            events.Cell(config_dict={"canbeblank": False}, cell="string", formatting_error="0"),
            events.Cell(config_dict={"canbeblank": True}, cell="", formatting_error="0"),
        ]
    )
    stream = list(stream)
...
def test_create_file_match_error():
    ...
    assert event_with_file_match_error[0].match_error == "Failed to find a set of matching columns headers for " \
